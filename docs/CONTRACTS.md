...
- Typed proposals only (no arbitrary calls). The allowed actions are:
  ...
  Creating a disband proposal immediately activates the join lock so newcomers cannot buy in while a disband is pending; the lock is released on execution or when the proposal expires.
