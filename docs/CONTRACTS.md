...
- `setJoinPausedDAO(bool)` - pause or resume new member joins.
  ...
  - `setFeeCurveDAO(uint8 formula, uint256 slope, uint256 scale)` - adjust the fee curve without waiting for a proposal (callable when the templ is in dictatorship mode, otherwise only through proposals executed by governance). Mirrors the semantics of `createProposalSetFeeCurve` and emits `FeeCurveUpdated`.
...
- `feeCurve()` - auto-generated getter returning `(formula, slope, scale)` describing the fee curve currently in effect; `formula` matches the enum (`0` constant, `1` linear).
