...
- The factory records the deploying wallet as `factoryDeployer`, exposes the current `permissionless` mode, and emits `PermissionlessModeUpdated` whenever `setPermissionless(bool)` flips the flag. Permissionless mode defaults to `false`, meaning only `factoryDeployer` may call `createTempl`/`createTemplWithConfig`; once set to `true`, anyone can deploy templs while the protocol recipient and percent remain immutable for every instance minted through that factory.
