...
- POST `/templs` (create/register a TEMPL group)
  ...
  - Creates a new XMTP group with the priest (if `priestInboxId` is provided, uses that; otherwise derives inboxId from address using `generateInboxId`).
...
- POST `/join` (purchase check + add member to XMTP group)
  ...
  - Adds the member’s inboxId to the group (uses provided `memberInboxId` if present; otherwise derives from address with `generateInboxId`).
