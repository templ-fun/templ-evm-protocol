...
describe("Disband Treasury", function () {
  ...
  it("requires quorum when other members exist", async function () {
    const accessToken = await templ.accessToken();
    await templ.connect(priest).createProposalDisbandTreasury(accessToken, VOTING_PERIOD);
    const proposal = await templ.proposals(0);
    expect(proposal.quorumExempt).to.equal(false);
  });

  it("grants quorum exemption to the priest when they are the sole member", async function () {
    const solo = await deployTempl({ entryFee: ENTRY_FEE });
    const soloAccess = await solo.templ.accessToken();
    await solo.templ.connect(solo.priest).createProposalDisbandTreasury(soloAccess, VOTING_PERIOD);
    const proposal = await solo.templ.proposals(0);
  ...
  it("finalizes disband failures without requiring manual pruning", async function () {
    const accessToken = await templ.accessToken();
    const extraMember = accounts[5];
    const lateJoiner = accounts[6];
    const templAddress = await templ.getAddress();
