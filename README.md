...
- **Contracts** – Solidity 0.8.23 factory + templ modules (see `contracts/`). Features: configurable entry-fee splits (burn/treasury/member/protocol), auto-enrolment of the deploying priest (so the first paid member pool accrues to them), an on-chain home link for off-chain surfaces, and a typed governance router (pause/config/withdraw/disband/priest/cap/dictatorship/home-link). Disband proposals no longer pause membership—only explicit governance pauses or member caps stop new joins. External reward enumeration is capped at 256 tokens, and anyone can call `cleanupExternalRewardToken(token)` once balances are settled to free a slot. Fee accounting assumes standard ERC-20 transfers; taxed tokens are unsupported.
