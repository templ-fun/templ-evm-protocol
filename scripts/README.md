...
- `deploy-cloudflare.js` - apply the D1 schema (optional), build the SPA with production env vars, and push the site to Cloudflare Pages from a single env file. The backend must be deployed separately now that Workers hosting has been retired.
...
- Generates `backend/wrangler.deployment.toml`, applies the D1 schema, and deploys the static frontend to Pages. Run it with `--skip-worker` (enforced) and deploy the Node backend yourself.
